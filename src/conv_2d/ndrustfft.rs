...
use ndarray::prelude::*;
use num::{traits::NumAssign, ToPrimitive, traits::AsPrimitive};
    ...
    f64: std::convert::From<T>,
{
    ...
    let mut data_ext = data_ext.mapv(|x| x.as_());
    let mut buf = Array2::<Complex<f32>>::zeros((nx / 2 + 1, ny));
    let mut data_spec = Array2::<Complex<f32>>::zeros((nx / 2 + 1, ny));
    let mut fft_handler_x = R2cFftHandler::<f32>::new(nx);
    let mut fft_handler_y = FftHandler::<f32>::new(ny);
    ndfft_r2c(&data_ext.view(), &mut buf.view_mut(), &mut fft_handler_x, 0);
    ndfft(
        &buf.view(),
    ...
    let mut kernel_ext = kernel_ext.mapv(|x| x.as_());
    let mut kernel_spec = Array2::<Complex<f32>>::zeros((nx / 2 + 1, ny));
    ...
    let mut mul_spec = data_spec * kernel_spec;
    let mut ret_buf = Array2::zeros((nx / 2 + 1, ny));
    let mut ret = Array2::zeros(data_ext.dim());
    ndifft(&mul_spec.view(), &mut ret_buf, &mut fft_handler_y, 1);
    ndifft_r2c(&ret_buf.view(), &mut ret, &mut fft_handler_x, 0);

    Some(ret.mapv(|x| T::from_f32(x.round()).unwrap()))
}
