    ...
    f64: std::convert::From<T>,
{
    ...
    let mut data_ext = data_ext.mapv(|x| x.into());
    ...
    ndfft(&data_ext.view(), &mut buf.view_mut(), &mut fft_handler_x, 0);
    ndfft(
        &buf.view(),
    ...
    let mut kernel_ext = kernel_ext.mapv(|x| x.into());
    ...
    let mut mul_spec = data_spec * kernel_spec;
    let mut ret_buf = Array2::zeros((nx, ny));
